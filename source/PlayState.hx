...
class PlayState extends MusicBeatState
{
	...
	//ALL VS DARK VARS! KILL ME NOW - TRSF2.0
	...
	var darkCity:BGSprite;
	var darkSidewalk:BGSprite;
	var darkLamp1:BGSprite;
	var darkLamp2:BGSprite;
	var darkFence:BGSprite;
	var darkGround:BGSprite;
	var darkFlicker:BGSprite;
	var memBG1:BGSprite;
	var memBG2:BGSprite;
	...
	var thirdCharCreated:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'starryAlley':
				var darkbg:BGSprite = new BGSprite('alley/sky', -580, 210, 0.9, 0.9);
				darkbg.setGraphicSize(Std.int(darkbg.width * 2));
				add(darkbg);
				var city:BGSprite = new BGSprite('alley/city', -590, 200, 0.9, 0.9);
				add(city);
				var sidewalk:BGSprite = new BGSprite('alley/sidewalk', -560, 970, 0.9, 0.9);
				add(sidewalk);
				var lamp1:BGSprite = new BGSprite('alley/lamp broken', -280, 450, 0.9, 0.9);
				add(lamp1);
				var lamp2:BGSprite = new BGSprite('alley/lamp', 890, 450, 0.9, 0.9);
				add(lamp2);
				var fence:BGSprite = new BGSprite('alley/fence', -610, 700, 0.9, 0.9);
				add(fence);
				var ground:BGSprite = new BGSprite('alley/ground', -540, 1100, 0.9, 0.9);
				add(ground);
			...
			case 'mementos':
				...
				add(darkSky);
				 darkCity = new BGSprite('alley/city', -590, 200, 0.9, 0.9);
				add(darkCity);
				 darkSidewalk = new BGSprite('alley/sidewalk', -560, 970, 0.9, 0.9);
				add(darkSidewalk);
				 darkLamp1 = new BGSprite('alley/lamp broken', -280, 450, 0.9, 0.9);
				add(darkLamp1);
				 darkLamp2 = new BGSprite('alley/lamp', 890, 450, 0.9, 0.9);
				add(darkLamp2);
				 darkFence = new BGSprite('alley/fence', -610, 700, 0.9, 0.9);
				add(darkFence);
				 darkGround = new BGSprite('alley/ground', -540, 1100, 0.9, 0.9);
				add(darkGround);
				darkFlicker = new BGSprite('alley/light_flicker', 687, 490, 0.9, 0.9, ['flicker'], true);
				add(darkFlicker);
				oldDarkBGR = new BGSprite('mementos/dark_bg', -800, 125, 0.9, 0.9); //57 stage size //256
				oldDarkBGR.visible = false;
				add(oldDarkBGR);
				memBG1 = new BGSprite('mementos/park_bg', -175, 438, 0.9, 0.9); //128
				memBG1.setGraphicSize(Std.int(memBG1.width * 2.13));
				memBG1.visible = false;
				add(memBG1);
				memBG2 = new BGSprite('mementos/our_start', -812, 100, 0.9, 0.9); //64
				memBG2.setGraphicSize(Std.int(memBG2.width * 1.07));
				memBG2.scale.y = 1.15;
				memBG2.visible = false;
				add(memBG2);
		...
		iconP3 = new HealthIcon(dark.healthIcon, false);
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(tmr.loopsLeft % 2 == 0) {
					...
					if (dark.animation.curAnim != null && !dark.animation.curAnim.name.startsWith('sing') && !dark.stunned)
						{
				...
				else if(dark.danceIdle && dark.animation.curAnim != null && !dark.stunned && !dark.animation.curAnim.name.startsWith("sing"))
				{
					if(!dad.animation.curAnim.name.startsWith('sing') && !dad.stunned) {
						dark.dance();
					} else {
	...
	override public function update(elapsed:Float)
	{
		...
		if(curSong.toLowerCase() == 'antivoid') {
			iconP2.x = (healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset * 2) - 75;
		} else {
			iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset * 2;
		}
		iconP3.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP3.scale.x) / 2 - iconOffset * 2;
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				'week5_nomiss', 'week6_nomiss', 'weekdark_nomiss', 'ur_bad',
	...
	override function beatHit()
	{
		...
		if(curSong.toLowerCase() == 'mirror' && curBeat == 64)
		{	
			FlxG.camera.flash(FlxColor.PURPLE, 1);
			defaultCamZoom = 0.7;
			darkCity.visible = false;
			darkFence.visible = false;
			darkGround.visible = false;
			darkLamp1.visible = false;
			darkLamp2.visible = false;
			darkSidewalk.visible = false;
			darkFlicker.visible = false;
			darkSky.visible = false;
			memBG2.visible = true;
		}

		if(curSong.toLowerCase() == 'mirror' && curBeat == 128)
		{	
			FlxG.camera.flash(FlxColor.PURPLE, 1);
			darkCity.visible = false;
			darkFence.visible = false;
		...
		if(curSong.toLowerCase() == 'mirror' && curBeat == 256)
		{	
			FlxG.camera.flash(FlxColor.PURPLE, 1);
			memBG1.visible = false;
			oldDarkBGR.visible = true;
		}


		if(curSong.toLowerCase() == 'mirror' && curBeat == 320)
		{	
			FlxG.camera.flash(FlxColor.PURPLE, 1);
			defaultCamZoom = 0.9;
			oldDarkBGR.visible = false;
			darkCity.visible = true;
			darkFence.visible = true;
			darkGround.visible = true;
			darkLamp1.visible = true;
			darkLamp2.visible = true;
			darkSidewalk.visible = true;
			darkFlicker.visible = true;
			darkSky.visible = true;
		}
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'darkweek_nomiss':
